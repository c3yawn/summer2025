...
def lambda_handler(event, context):
    try:
        logger.info(f"Python compiler invoked with request ID: {context.aws_request_id}")
        ...
        main_file = body.get('mainFileName', 'main.py')
        
        if not files:
            return create_api_response(create_response(False, '', 'No files provided'))
        
        logger.info(f"Executing {len(files)} Python files")
        result = execute_python(files, main_file)
...
def execute_python(files, main_file):
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for file_data in files:
                ...
                if filename == main_file or (main_file_path is None and filename.endswith('.py')):
                    main_file_path = file_path
                
                logger.info(f"Created file: {filename} ({len(content)} chars)")
            
            if not main_file_path:
                return create_response(False, '', 'No Python file found to execute')
            
            logger.info(f"Executing Python file: {os.path.basename(main_file_path)}")
            
            run_result = subprocess.run(
                ['python3', os.path.basename(main_file_path)],
                capture_output=True,
                text=True,
                timeout=12,
                cwd=temp_dir
